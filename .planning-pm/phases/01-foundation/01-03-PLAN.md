...
```bash
grep -n "migrateToNested\|verifyMigration" get-shit-done/bin/gsd-tools.js | head -10
```

Verify project migrate subcommand exists:
```bash
grep -n "'migrate'" get-shit-done/bin/gsd-tools.js
```
...
```bash
node ./get-shit-done/bin/gsd-tools.js project create "Test Project Alpha" "A test project for verification"
```
...
```bash
node ./get-shit-done/bin/gsd-tools.js project switch "test-project-alpha"
...
<verification>
1. `grep -c "migrateToNested" get-shit-done/bin/gsd-tools.js` returns 2+ (function def + usage)
2. `node ./get-shit-done/bin/gsd-tools.js project list --raw` returns valid JSON
