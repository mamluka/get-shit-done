...
class PathResolver {
  ...
  /**
   * Detect operating mode by checking .active-project file or STATE.md for project/version fields
   * @returns {'flat'|'nested'}
   */
  detectMode() {
    // Check for .active-project file first (new nested mode indicator)
    const activeProjectPath = path.join(this.planningRoot, '.active-project');
    if (fs.existsSync(activeProjectPath)) {
      return 'nested';
    }

    // Fall back to checking STATE.md for backward compatibility
   ...
   */
  loadProjectContext() {
    // First check .active-project file
    const activeProjectPath = path.join(this.planningRoot, '.active-project');

    if (fs.existsSync(activeProjectPath)) {
      try {
        const slug = fs.readFileSync(activeProjectPath, 'utf-8').trim();
        if (slug) {
          this.currentProject = slug;

          // Find latest version in project directory
          const projectPath = path.join(this.planningRoot, slug);
          if (fs.existsSync(projectPath)) {
            ...
            this.currentVersion = versions[0] || 'v1';
            return;
          }
        }
      } catch (err) {
        // Fall through to STATE.md check
      }
    }

    // Fall back to reading STATE.md (backward compatibility)
    const statePath = path.join(this.planningRoot, 'STATE.md');
...
function getActiveProjectName(cwd) {
  ...
  try {
    ...
    return headingMatch ? headingMatch[1].trim() : activeSlug;
  } catch {
    return activeSlug;
  }
}
...
function detectFlatStructure(cwd) {
  ...
  // Flat structure: STATE.md exists at root, no .active-project, no project subdirectories
  if (!fs.existsSync(statePath)) return false;
  if (fs.existsSync(activeProjectPath)) return false;

  // Check for project subdirectories (directories with PROJECT.md)
  try {
...
function createProjectInternal(cwd, friendlyName, description) {
  if (!friendlyName) {
    return { error: 'Friendly name is required' };
  }

  // Generate slug with validation
  let slug = generateSlugInternal(friendlyName);
  if (!slug || slug.length === 0) {
    return { error: 'Could not generate valid slug from name' };
  }

  // Truncate to 50 chars and remove trailing hyphen
  if (slug.length > 50) {
  ...
  if (reserved.includes(slug)) {
    return { error: `"${slug}" is a reserved name and cannot be used` };
  }
  ...
  const projectMdContent = `# ${friendlyName}
...
`;
  ...
  // Create STATE.md with standard template
  const stateMdContent = `# Project State

## Project Reference

See: .planning/${finalSlug}/PROJECT.md (updated ${created})
...
Resume file: None
`;
  fs.writeFileSync(path.join(v1Path, 'STATE.md'), stateMdContent);
...
[To be planned]
`;
  fs.writeFileSync(path.join(v1Path, 'ROADMAP.md'), roadmapContent);
...
[To be defined]
`;
  fs.writeFileSync(path.join(v1Path, 'REQUIREMENTS.md'), requirementsContent);

  // Create empty config.json
  fs.writeFileSync(path.join(v1Path, 'config.json'), '{}');
...
function switchProjectInternal(cwd, projectSlug) {
  ...
  const planningDir = path.join(cwd, '.planning');
  const projectPath = path.join(planningDir, projectSlug);
  ...
  const activeProjectPath = path.join(planningDir, '.active-project');
  fs.writeFileSync(activeProjectPath, projectSlug);
...
function listProjectsInternal(cwd) {
  ...
  const excluded = ['_backup', 'codebase', 'research', 'phases', 'quick', 'todos', 'archive'];
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      // Read friendly name and description
      const content = fs.readFileSync(projectMd, 'utf-8');
      const nameMatch = content.match(/^#\s+(.+)$/m);
      const descMatch = content.match(/\*\*Description:\*\*\s+(.+)$/m);

      // Find latest version
      const projectPath = path.join(planningDir, entry.name);
...
function cmdInitCreateProject(cwd, raw) {
  ...
  const existingProjects = listProjectsInternal(cwd);
  const hasFlatStructure = detectFlatStructure(cwd);
  const config = loadConfig(cwd);

  const result = {
    planning_exists: planningExists,
    existing_projects: existingProjects,
    has_flat_structure: hasFlatStructure,
...
function cmdProjectCreate(cwd, friendlyName, description, raw) {
  const result = createProjectInternal(cwd, friendlyName, description);
  output(result, raw);
}

function cmdProjectSwitch(cwd, projectSlug, raw) {
  const result = switchProjectInternal(cwd, projectSlug);
  output(result, raw);
}
  ...
  switch (command) {
    ...
    case 'project': {
      ...
      if (subcommand === 'create') {
        cmdProjectCreate(cwd, args[2], args[3], raw);
