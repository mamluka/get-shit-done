...
/**
 * Load Notion configuration from .planning-pm/config.json
 *
...
function loadNotionConfig(cwd) {
  const configPath = path.join(cwd, '.planning-pm', 'config.json');

  // Check if config.json exists
  if (!fs.existsSync(configPath)) {
    throw new Error('No .planning-pm/config.json found. Run npx @david-xpn/gsd-pm to set up.');
  }
  ...
  try {
    ...
    throw new Error(`Failed to parse .planning-pm/config.json: ${e.message}`);
  }

  // Check if notion section exists
  if (!config.notion) {
    throw new Error('No Notion configuration in config.json. Add notion.api_key to .planning-pm/config.json');
  }

  // Check if api_key exists and is not empty
  if (!config.notion.api_key || config.notion.api_key.trim() === '') {
    throw new Error('Notion API key not found. Add notion.api_key to .planning-pm/config.json');
  }
