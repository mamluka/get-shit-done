...
function convertMarkdown(markdown, options = {}) {
  ...
  try {
    ...
    // Step 5.5: Strip empty children arrays from blocks that don't support them
    blocks = stripInvalidChildren(blocks);
...
/**
 * Block types that support children in the Notion API.
 */
const CHILD_SUPPORTING_TYPES = new Set([
  'bulleted_list_item', 'numbered_list_item', 'to_do',
  'toggle', 'callout', 'quote', 'synced_block',
  'template', 'column', 'column_list', 'table'
]);

/**
 * Recursively strip empty `children` arrays from blocks that don't support them.
 * The Notion API rejects blocks like `code` that have `children: []`.
 *
 * @param {Array} blocks - Array of Notion blocks
 * @returns {Array} - Cleaned blocks
 */
function stripInvalidChildren(blocks) {
  for (const block of blocks) {
    const content = block[block.type];
    if (!content) continue;

    if (CHILD_SUPPORTING_TYPES.has(block.type)) {
      // Recursively clean children
      if (content.children && content.children.length > 0) {
        stripInvalidChildren(content.children);
      }
    } else {
      // Remove children from block types that don't support them
