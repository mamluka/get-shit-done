...
```bash
VALIDATION=$(node ~/.claude/gsd-pm/bin/gsd-tools.js phase validate "${PHASE_NUM}" --raw)
```
...
```bash
COMPLETE_RESULT=$(node ~/.claude/gsd-pm/bin/gsd-tools.js phase complete "${PHASE_NUM}" --raw)
```
...
```bash
node ~/.claude/gsd-pm/bin/gsd-tools.js commit "docs(phase-${completed_phase}): mark phase complete" --files .planning-pm/ROADMAP.md .planning-pm/STATE.md
```
