...
```bash
INIT=$(node ~/.claude/gsd-pm/bin/gsd-tools.js init plan-phase "${PHASE_NUM}" --raw)
```
...
```bash
node ~/.claude/gsd-pm/bin/gsd-tools.js commit "docs(phase-${phase_number}): revise ${ARTIFACT_TYPE}" --files ${MODIFIED_FILES[@]}
```
