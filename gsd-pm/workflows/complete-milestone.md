...
```bash
BRANCH_STATUS=$(node ~/.claude/gsd-pm/bin/gsd-tools.js git status --raw)
```
...
for summary in .planning-pm/phases/*-*/*-SUMMARY.md; do
  node ~/.claude/gsd-pm/bin/gsd-tools.js summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
```bash
ARCHIVE=$(node ~/.claude/gsd-pm/bin/gsd-tools.js milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
```bash
INIT=$(node ~/.claude/gsd-pm/bin/gsd-tools.js init milestone-op)
```
...
```bash
node ~/.claude/gsd-pm/bin/gsd-tools.js commit "chore: complete v[X.Y] milestone" --files .planning-pm/milestones/v[X.Y]-ROADMAP.md .planning-pm/milestones/v[X.Y]-REQUIREMENTS.md .planning-pm/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning-pm/MILESTONES.md .planning-pm/PROJECT.md .planning-pm/STATE.md
```
```
