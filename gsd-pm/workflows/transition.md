...
cat .planning-pm/PROJECT.md 2>/dev/null
```
...
```bash
ls .planning-pm/phases/XX-current/*-PLAN.md 2>/dev/null | sort
ls .planning-pm/phases/XX-current/*-SUMMARY.md 2>/dev/null | sort
```
...
```bash
ls .planning-pm/phases/XX-current/.continue-here*.md 2>/dev/null
```
...
```bash
TRANSITION=$(node ~/.claude/gsd-pm/bin/gsd-tools.js phase complete "${current_phase}")
```
...
```bash
cat .planning-pm/phases/XX-current/*-SUMMARY.md
```
...
```bash
PROGRESS=$(node ~/.claude/gsd-pm/bin/gsd-tools.js progress bar --raw)
```
...
Exit skill and invoke SlashCommand("/gsd-pm:plan-phase [X+1]")
...
**Also available:**
- `/gsd-pm:discuss-phase [X+1]` — gather context first
- `/gsd-pm:research-phase [X+1]` — investigate unknowns
- Review roadmap
...
Exit skill and invoke SlashCommand("/gsd-pm:complete-milestone {version}")
