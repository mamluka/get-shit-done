...
- Example: `/gsd-pm:remove-phase 17` → phase = 17
- Example: `/gsd-pm:remove-phase 16.1` → phase = 16.1

If no argument provided:

```
ERROR: Phase number required
Usage: /gsd-pm:remove-phase <phase-number>
Example: /gsd-pm:remove-phase 17
```
...
```bash
INIT=$(node ~/.claude/gsd-pm/bin/gsd-tools.js init phase-op "${target}")
```
...
To abandon current work, use /gsd-pm:pause-work instead.
```
...
This will:
- Delete: .planning-pm/phases/{target}-{slug}/
...
```bash
RESULT=$(node ~/.claude/gsd-pm/bin/gsd-tools.js phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node ~/.claude/gsd-pm/bin/gsd-tools.js phase remove "${target}" --force)
```
...
```bash
node ~/.claude/gsd-pm/bin/gsd-tools.js commit "chore: remove phase {target} ({original-phase-name})" --files .planning-pm/
```
...
Changes:
- Deleted: .planning-pm/phases/{target}-{slug}/
...
Would you like to:
- `/gsd-pm:progress` — see updated roadmap status
