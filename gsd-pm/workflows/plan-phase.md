...
```bash
INIT=$(node ~/.claude/gsd-pm/bin/gsd-tools.js init plan-phase "$PHASE" --include state,roadmap,requirements,context,research,verification,uat,planning-status)
```
...
```bash
node ~/.claude/gsd-pm/bin/gsd-tools.js planning-status init
```
...
```bash
PHASE_INFO=$(node ~/.claude/gsd-pm/bin/gsd-tools.js roadmap get-phase "${PHASE}")
```
...
```
Task(
  prompt="First, read ~/.claude/gsd-pm/workflows/discuss-phase.md for your role and instructions.\n\n<context>\nPhase number: {phase_number}\n\nLoad project state:\n@.planning-pm/STATE.md\n\nLoad roadmap:\n@.planning-pm/ROADMAP.md\n</context>",
...
```bash
node ~/.claude/gsd-pm/bin/gsd-tools.js planning-status mark-in-progress --phase "${PHASE}"
```
...
```bash
PHASE_DESC=$(node ~/.claude/gsd-pm/bin/gsd-tools.js roadmap get-phase "${PHASE}" | jq -r '.section')
...
STATE_SNAP=$(node ~/.claude/gsd-pm/bin/gsd-tools.js state-snapshot)
...
```bash
VALIDATION=$(node ~/.claude/gsd-pm/bin/gsd-tools.js phase validate "${PHASE}" --raw)
```
...
```bash
COMPLETE_RESULT=$(node ~/.claude/gsd-pm/bin/gsd-tools.js phase complete "${PHASE}" --raw)
```
...
```bash
node ~/.claude/gsd-pm/bin/gsd-tools.js planning-status update --phase "${PHASE}" --status done --plans "${PLAN_COUNT}"
```

### 14c. Commit Completion

```bash
node ~/.claude/gsd-pm/bin/gsd-tools.js commit "docs(phase-${PHASE}): mark phase complete" --files .planning-pm/ROADMAP.md .planning-pm/STATE.md
```
...
if node ~/.claude/gsd-pm/bin/notion-sync.js sync --cwd "$(pwd)"; then
  echo ""
