...
```bash
node ~/.claude/gsd-pm/bin/notion-sync.js comments --cwd "$(pwd)"
```
...
```bash
node ~/.claude/gsd-pm/bin/gsd-tools.js commit "docs(16): incorporate comment feedback into planning artifacts" --files .planning-pm/ROADMAP.md .planning-pm/REQUIREMENTS.md
```
