...
if [ -f "./.claude/gsd-pm/VERSION" ]; then
  cat "./.claude/gsd-pm/VERSION"
  echo "LOCAL"
elif [ -f ~/.claude/gsd-pm/VERSION ]; then
  cat ~/.claude/gsd-pm/VERSION
  echo "GLOBAL"
else
  echo "UNKNOWN"
...
- `gsd-pm/` will be wiped and replaced
...
[View full changelog](https://github.com/glittercowboy/gsd-pm/blob/main/CHANGELOG.md)
```
</step>
