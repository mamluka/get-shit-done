...
```bash
PHASE_INFO=$(node ~/.claude/gsd-pm/bin/gsd-tools.js find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/gsd-pm/bin/gsd-tools.js roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/gsd-pm/bin/gsd-tools.js find-phase "${PHASE}" --raw)
```
