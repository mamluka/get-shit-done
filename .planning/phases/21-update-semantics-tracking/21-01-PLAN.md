...
No changes needed -- assignment operates on whatever tickets exist after create/update.
...
8. create_tickets (was 7, now with update logic)
9. assign_team (was 8)
...
5. save_sync_state step uses saveSyncState and merges with existing state
6. No steps were accidentally deleted

Run: `grep -c '<step name=' get-shit-done/workflows/sync-jira.md` -- should output 10
  </verify>
  <done>
sync-jira.md has 10 steps. detect_existing_sync step loads jira-sync.json and determines fresh vs incremental mode. preview_and_approve shows create vs update counts. create_tickets routes to createJiraIssue for new tickets and editJiraIssue for existing tickets. save_sync_state merges new state with existing state preserving unmodified tickets. Granularity metadata (phase/category/requirement_id) is included on every ticket in jira-sync.json.
  </done>
</task>
...
4. save_sync_state step with merge logic preserving existing ticket data
  </action>
  <verify>
...
3. Verify jira-sync.json contains granularity metadata on each ticket
  </verify>
  <done>
User confirms: re-running sync-jira updates existing tickets without creating duplicates. Preview correctly shows create vs update counts. jira-sync.json contains granularity metadata on all tickets. All SYNC requirements (01, 02, 03) are satisfied.
  </done>
</task>
...
5. jira-sync.json tracks all tickets with granularity metadata
6. No duplicates created on re-run
</verification>
