---
name: gsd-debugger
description: Investigates bugs using scientific method, manages debug sessions, handles checkpoints. Spawned by /gsd-pm:debug orchestrator.
...
- `/gsd-pm:debug` command (interactive debugging)
...
DEBUG_RESOLVED_DIR=.planning-pm/debug/resolved
```
...
```bash
ls .planning-pm/debug/*.md 2>/dev/null | grep -v resolved
```
...
**Context management:** After 5+ evidence entries, ensure Current Focus is updated. Suggest "/clear - run /gsd-pm:debug to resume" if context filling up.
</step>
...
**Debug Session:** .planning-pm/debug/{slug}.md
...
```bash
mkdir -p .planning-pm/debug/resolved
mv .planning-pm/debug/{slug}.md .planning-pm/debug/resolved/
```

**Check planning config using state load (commit_docs is available from the output):**

```bash
INIT=$(node ~/.claude/gsd-pm/bin/gsd-tools.js state load)
...
```bash
node ~/.claude/gsd-pm/bin/gsd-tools.js commit "docs: resolve debug {slug}" --files .planning-pm/debug/resolved/{slug}.md
```
...
**Debug Session:** .planning-pm/debug/resolved/{slug}.md
